...
void NonBlockingDialogs::startKeepAlive()
{
    ...
    if (!keepAliveTimer) {
        ...
        QObject::connect(keepAliveTimer, &QTimer::timeout, [](){
            // 只处理必要的事件，避免影响WebSocket消息处理顺序
            // 排除用户输入和Socket通知，只处理定时器和绘制事件
            QApplication::processEvents(QEventLoop::ExcludeUserInputEvents, 50);
        });
    }
    if (!keepAliveTimer->isActive()) {
        keepAliveTimer->start(500); // 减少到500ms，但限制处理类型
    }
}
...
void NonBlockingDialogs::getSaveFileName(QWidget* parent, const QString& caption,
...
void NonBlockingDialogs::getOpenFileName(QWidget* parent, const QString& caption,
                                        const QString& dir, const QString& filter,
                                        std::function<void(const QString&)> callback)
{
    QFileDialog* dialog = new QFileDialog(parent);
...
void NonBlockingDialogs::getExistingDirectory(QWidget* parent, const QString& caption,
                                            const QString& dir,
                                            std::function<void(const QString&)> callback)
{
    QFileDialog* dialog = new QFileDialog(parent);
...
void NonBlockingDialogs::getText(QWidget* parent, const QString& title, const QString& label,
...
void NonBlockingDialogs::getInt(QWidget* parent, const QString& title, const QString& label,
...
void NonBlockingDialogs::question(QWidget* parent, const QString& title, const QString& text,
...
void NonBlockingDialogs::information(QWidget* parent, const QString& title, const QString& text,
...
void NonBlockingDialogs::critical(QWidget* parent, const QString& title, const QString& text,
                                 std::function<void()> callback)
{
    QMessageBox* msgBox = new QMessageBox(parent);
    msgBox->setWindowTitle(title);
