...
void DialogAgent::createUI()
{
    ...
    listenerDisplayEdit->setPlaceholderText("点击选择监听器...");

    listenerSelectBtn = new QPushButton("...", this);
    listenerSelectBtn->setToolTip("选择监听器");
    ...
    btnMoveUp->setToolTip("向上移动所选监听器");
    btnMoveDown = new QPushButton("↓");
    btnMoveDown->setToolTip("向下移动所选监听器");
    ...
    buildButton = new QPushButton("生成", this);
    ...
    collapseButton = new QPushButton(QIcon(":/icons/arrow_drop_down"), " 构建日志", this);
...
//
//         NonBlockingDialogs::getSaveFileName(safeThis, "保存文件", initialPath, "All Files (*.*)",
...
//                 inputDialog.setLabelText("文件已保存至:");
    ...
    NonBlockingDialogs::getOpenFileName(this, "选择文件", baseDir, "JSON files (*.json)",
        [safeThis, currentListenerType](const QString& filePath) {
            ...
            if ( !jsonObject.contains("listener_type") || !jsonObject["listener_type"].isString() ) {
                MessageError("缺少必需参数 'listener_type'");
                return;
            }
            if ( !jsonObject.contains("agent") || !jsonObject["agent"].isString() ) {
                MessageError("缺少必需参数 'agent'");
                return;
            }
...
void DialogAgent::changeConfig(const QString &agentName)
{
    ...
    listenerLabel->setText(isMultiListeners ? "监听器：" : "监听器：");
...
void DialogAgent::onProfileRename()
{
    ...
    bool ok;
    QString newName = QInputDialog::getText(this, "重命名配置", "新配置名称:", QLineEdit::Normal, oldName, &ok);
...
void DialogAgent::onBuildMessage(const QString &msg)
{
    ...
    switch (status) {
        ...
        case 4: { // BUILD_LOG_SAVE_FILE
            ...
            QPointer<DialogAgent> safeThis = this;
            NonBlockingDialogs::getSaveFileName(this, "保存文件", initialPath, "All Files (*.*)",
                [safeThis, content](const QString& filePath) {
                    if (filePath.isEmpty())
                        return;
